...
public class BattleshipGame{
    public static void main(String[] args) {
        ...
        while(undetectedShipNumber > 0){
            int [] guessCoodinates = getUserCoodinates(gameBoardLength);
            char locationViewUpdate = evaluateGuessAndGetTheTarget(guessCoodinates, gameBoard, ship, water, hit, miss);

            if(locationViewUpdate == hit){
                undetectedShipNumber--;
            }
            gameBoard = updateGameBoard(gameBoard, guessCoodinates, locationViewUpdate);
            printGameBoard(gameBoard, water, ship);
        }
    ...
    private static char[][] updateGameBoard(char[][] gameBoard, int[] guessCoodinates, char locationViewUpdate) {
        int row = guessCoodinates[0];
        int col = guessCoodinates[1];
        gameBoard[row][col] = locationViewUpdate;
        return gameBoard;
    }

    private static char evaluateGuessAndGetTheTarget(int[] guessCoodinates, char[][] gameBoard, char ship, char water,
            char hit, char miss) {
    ...
    private static int[] getUserCoodinates(int gameBoardLength) {
        int row;
        int col;
        do{
    ...
    private static char[][] placeShips(char[][] gameBoard, int shipNumber, char water, char ship) {
        ...
        int gameBoardLength = gameBoard.length;
            while(placedShips < shipNumber){
                int[] location = generateShipCoodinates(gameBoardLength);
    ...
    private static int[] generateShipCoodinates(int gameBoardLength) {
